...
@IonicPage()
@Component({
  selector: 'page-vendor-agents',
  templateUrl: 'vendor-agents.html',
})
export class VendorAgentsPage {
  ...
  fetchSuppliers(isPaging: boolean, isRefresh: boolean, isFirst: boolean, paging, refresher) {
    try {
      ...
      let url = this.apiService.getSuppliers() + this.user.userid + "/" + APP_TYPE + "/" + this.user.usertype;
